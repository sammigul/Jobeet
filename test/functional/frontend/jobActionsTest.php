...
$browser->
  ...
  get('/it/')->
  with('response')->isStatusCode(404) // 404 bcz it is not available culture
;

$browser->setHttpHeader('ACCEPT_LANGUAGE', 'en,fr;q=0.7'); // setting http header for the very next req. doing this bcz symfony can't simulate js
$browser->
  ...
  with('response')->isRedirected()->
  followRedirect()->
  with('user')->isCulture('fr')  // user culture is still fr
;